...
// async function handleNoneOfTheAbove () {
//   console.log("function called !!!!!!!!!!!")
// }

async function checkAllergie(allergy){
  console.log("allergie function called !!!!!!! ")
  let arrayOfAllergies =  ["Banana", "Olive", "Sunflowers"]
  if(arrayOfAllergies.includes(allergy)){
    //end quiz here.... 
    terminateQuiz();
  }
...
$(document).on("click", ".beginButton", async function () {
  ...
  await getQuizzesApi();
  await terminateConfigurationDynamic()
  showQuizTitle();
  return nextQuestion();
});
async function terminateConfigurationDynamic () {
  $.ajax({
...
async function askQuestion(totalQuizQuestions, counter, fromBack) {
  ...
  if (type == 1) {
    ...
    let questionId = Math.random()
    $("#typeSelection .answerInner").html("");
    $("#typeSelection").css("display", "block");
    currentActiveAnswerType = "typeSelection";
    ques.answers.push({answer: 'None of the above', id:questionId})

    ques.answers.forEach((val, index) => {
      if (val.answer && val.id) {
        if(val.id == questionId){
          $("#typeSelection .answerInner").append(`
          <div class="selectionOptions">
            <button  data-val="${val.answer}" data-id="${val.id}" class="selectionBtns selectionBtn" onclick="handleNoneOfTheAbove()">${val.answer}</button>
          </div>
        `);
        }
        else{
